...
@TestPropertySource(locations = "classpath:application-test.properties")  // 설정
class ItemRepositoryTest {
    ...
    public void queryDslTest(){
        this.createItemList();
        /*이렇게 조합해서 할 수도 있음(쿼리문의 경우) -> findBy로 해도 됨!*/
        JPAQueryFactory queryFactory = new JPAQueryFactory(em);
        QItem qItem = QItem.item;     // QItem 클래스 들어가보기
    ...
    public void createItemList2(){
        for(int i = 1; i <= 5; i++){   // 1~5번 상품 판매상태 : SELL
            Item item = new Item();
            item.setItemNm("테스트 상품" + i);
            item.setPrice(10000 + i);
            item.setItemDetail("테스트 상품 상세 설명" + i);
        ...
        for(int i = 6; i <= 10; i++){   // 1~5번 상품 판매상태 : SOLD_OUT
            Item item = new Item();
            item.setItemNm("테스트 상품" + i);
            item.setPrice(10000 + i);
            item.setItemDetail("테스트 상품 상세 설명" + i);
